          ...
          <Autocomplete
            ...
            getOptionLabel={(selectedOption) => {
              if (selectedOption === '') {
                ...
                return (
                  selectedOption[fieldParams['selectLabel']] ||
                  selectedOption.label ||
                  ''
                );
              } else {
                const displayOption = fieldParams['options'].find(
                  (option) =>
                    option[fieldParams['selectValue']] === selectedOption ||
                    option.value === selectedOption
                );
                if (displayOption) {
                  return (
                    displayOption[fieldParams['selectLabel']] ||
                    displayOption.label
                  );
                } else {
                  console.error(
                    `ERROR: Wrong setup for field ${fieldName} - check "selectValue" and "selectLabel" definitions.`
                  );
                  return '?';
                }
              }
            ...
            isOptionEqualToValue={(option, value) => {
              if (option && value) {
                return (
                  option[fieldParams['selectValue']] === value ||
                  option.value === value
                );
              }
              return false;
            ...
            onChange={(_, newValue) => {
              const selectedValue = newValue
                ? newValue[fieldParams['selectValue']] || newValue.value
                : null;
              field.onChange(selectedValue);

              // Clear dependent fields if specified
              if (
                fieldParams.clearFieldsOnChange &&
                Array.isArray(fieldParams.clearFieldsOnChange)
              ) {
                fieldParams.clearFieldsOnChange.forEach(
                  (dependentFieldName) => {
