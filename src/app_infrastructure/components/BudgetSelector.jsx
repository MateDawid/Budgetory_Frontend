...
const BudgetSelector = () => {
  ...
  useEffect(() => {
    const loadContextBudget = () => {
      ...
      const storageContextBudgetId = localStorage.getItem(
        'budgetory.contextBudget'
      )
        ? parseInt(localStorage.getItem('budgetory.contextBudget'), 10)
        : null;
      const storageContextBudgetCurrency =
        localStorage.getItem('budgetory.contextBudgetCurrency') || '';
    ...
    const loadBudgets = async () => {
      try {
        ...
        const apiResponse = await getApiObjectsList(
          `${process.env.REACT_APP_BACKEND_URL}/api/budgets/`
        );
        setBudgets(apiResponse);
        if (apiResponse.length < 1) {
          return;
        }
        const contextBudget = apiResponse.find(
          (budget) => budget.id === loadedBudgetId
        );
        if (contextBudget) {
          ...
          localStorage.setItem(
            'budgetory.contextBudgetCurrency',
  ...
  const handleChange = (event) => {
    setSelectedBudget(
      budgets.find((budget) => budget.id === event.target.value.id)
    );
  ...
  return (
    <FormControl sx={{ width: '90%' }}>
      <InputLabel sx={{ fontWeight: 700 }}>Budget</InputLabel>
      <Select
