...
jest.mock('../../../app_infrastructure/components/DataGrid/StyledDataGrid', () => {
  const MockReact = require('react');
  return function MockStyledDataGrid({ rows, columns, loading, slots, slotProps, checkboxSelection, ...props }) {
      ...
      <div data-testid="styled-data-grid" role="grid">
        ...
        {rows.map((row) => (
          <div key={row.id} data-testid={`row-${row.id}`}>
            ...
            {columns.map((col) => (
              <span key={`${row.id}-${col.field}`} data-testid={`cell-${row.id}-${col.field}`}>
                {col.renderCell ? col.renderCell({ value: row[col.field], row }) : 
                 col.valueFormatter ? col.valueFormatter(row[col.field]) : 
                 row[col.field]}
              </span>
            ))}
          </div>
...
jest.mock('../../../app_infrastructure/components/DataGrid/utils/renderHyperlink', () => {
  return jest.fn((prefix, params) => params.value);
});
jest.mock('../../../app_infrastructure/components/DataGrid/utils/FilterHandlers', () => ({
  mappedFilterOperators: {},
  formatFilterModel: jest.fn((model) => model),
}));
jest.mock('../../../app_infrastructure/components/DataGrid/utils/getSortFieldMapping', () => {
  return jest.fn(() => ({}));
});
jest.mock('../../../app_infrastructure/components/DataGrid/StyledGridActionsCellItem', () => {
...
describe('TransferDataGrid', () => {
  ...
  beforeEach(() => {
    ...
    getApiObjectsList.mockImplementation((url) => {
      ...
      if (url.includes('/categories/')) return Promise.resolve(mockCategoryOptions);
      if (url.includes('/deposits/')) return Promise.resolve(mockDepositOptions);
  ...
  describe('Component Rendering', () => {
    ...
    test('renders all column headers', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('column-category')).toHaveTextContent('Category');
        expect(screen.getByTestId('column-value')).toHaveTextContent('Value');
        expect(screen.getByTestId('column-description')).toHaveTextContent('Description');
    ...
    test('initializes with correct page size', async () => {
      ...
      await waitFor(() => {
        const dataFetchCalls = getApiObjectsList.mock.calls.filter(call => 
          call[0].includes('/incomes/') && call.length > 1
        );
        expect(dataFetchCalls.length).toBeGreaterThan(0);
        expect(dataFetchCalls[0][1]).toEqual(expect.objectContaining({ 
          pageSize: 10, 
          page: 0 
        }));
      });
  ...
  describe('Filtering', () => {
    test('initializes with empty filter model', async () => {
      const formatFilterModel = require('../../../app_infrastructure/components/DataGrid/utils/FilterHandlers').formatFilterModel;
  ...
  describe('Select Options', () => {
    ...
    test('handles failed options fetch gracefully', async () => {
      getApiObjectsList.mockImplementation((url) => {
        if (url.includes('/periods/')) return Promise.reject(new Error('Failed'));
        if (url.includes('/incomes/')) return Promise.resolve(mockTransfersData);
        return Promise.resolve([]);
      });
