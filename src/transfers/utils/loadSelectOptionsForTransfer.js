 ...
 */
const loadSelectOptionsForTransfer = async (contextBudgetId, categoryType, setPeriodOptions, setEntityOptions, setDepositOptions, setCategoryOptions, setAlert) => {
    try {
        const periodResponse = await getApiObjectsList(`${process.env.REACT_APP_BACKEND_URL}/api/budgets/${contextBudgetId}/periods/`)
        setPeriodOptions(periodResponse);
        const entityResponse = await getApiObjectsList(`${process.env.REACT_APP_BACKEND_URL}/api/budgets/${contextBudgetId}/entities/`)
        ...
        const categoryResponse = await getApiObjectsList(`${process.env.REACT_APP_BACKEND_URL}/api/budgets/${contextBudgetId}/categories/?category_type=${categoryType}`)
