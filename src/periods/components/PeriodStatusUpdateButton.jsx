...
const statusesMapping = {
  ...
  [PeriodStatuses.CLOSED]: {
    ...
    modalMessage:
      'After closing Period you will not be able to add more Transfers in it. Do you want to continue?',
  },
};
  ...
  const onSubmit = async () => {
    try {
      ...
      if (error instanceof ApiError) {
        ...
        setAlert({
          type: 'error',
          message: `Unexpected error occurred. Period "${objectName}" was not updated.`,
        });
      }
    } finally {
      setOpen(false);
    ...
    <>
      ...
      <StyledModal open={open} onClose={() => setOpen(false)}>
        <Box width={400} bgcolor="#F1F1F1" p={3} borderRadius={5}>
          ...
          >
            <Typography>
              {statusesMapping[newPeriodStatus].modalMessage}
            </Typography>
            <StyledButton
              type="submit"
              variant="contained"
